...
_NAMES = {
    ...
    "gcd": math.gcd,
    "lcm": getattr(
        math, "lcm", lambda a, b: abs(a * b) // math.gcd(a, b) if a and b else 0
    ),
    "exp": math.exp,
...
def _safe_eval(node):
    ...
    if isinstance(node, ast.Name):
        if node.id in _NAMES:
            ...
            if callable(val):
                raise ValueError(
                    f"'{node.id}' is a function — call it like {node.id}(...)"
                )
            return val
...
class Tools:
    ...
    async def calculate(
        ...
        display_expr = (
            expression.strip() if expression.strip() == cleaned_expr else cleaned_expr
        )

        card = _build_card(display_expr, result_display)
        await _emit(
            __event_emitter__,
            f"✅ {display_expr} = {result_display}",
            done=True,
            hidden=True,
        )
