...
| `FOLLOWUP_QUERIES_PER_CYCLE` | `2` | Proactive thread-pulling: after each cycle a cheap LLM pass over the passages just gathered proposes up to this many follow-up searches for specific studies, datasets, people, organisations, or counter-claims worth their own query. They join the queue alongside planned and gap-filling queries. `0` disables (plan + reactive gap-filling only). |
...
- **Faster / cheaper runs:** Lower `MAX_RESEARCH_CYCLES` to 2–3 and `MIN_SOURCES_PER_SECTION` to 1, reduce `SEARCH_RESULTS_PER_QUERY` to 5, set `MAX_LINK_HOPS_PER_CYCLE` and `FOLLOWUP_QUERIES_PER_CYCLE` to 0, set `REPORT_MODE` to `single`, and set `SKIP_PLAN_CONFIRMATION` to `True`. Section-by-section reports, multi-hop, and follow-up thread-pulling each add LLM calls and fetches, so these cut cost the most.
